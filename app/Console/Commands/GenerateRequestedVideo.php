...
class GenerateRequestedVideo extends Command
{
    ...
    public function handle()
    {
        ...
        if (!empty($requestedGreet)) {
            ...
            // var_dump('greetMediaFiles: ', json_encode($greetMediaFiles, JSON_PRETTY_PRINT));
            // var_dump('count($greetMediaFiles): ', count($greetMediaFiles));
            ...
            if (isset($greetMediaFiles) && sizeof($greetMediaFiles) > 0) {
                ...
                try {
                    ...
                    if ($isTheme) {
                        exec($ffmpegBinPath . ' -loop 1 -i ' . storage_path('app/public/theme_image/' . $greetTheme->file_name) . ' -preset faster -c:v libx264 -t 5 -pix_fmt yuv420p -vf scale=2400:1600,setsar=1 ' . $backgroundVideoPath);
                    }
                    ...
                    foreach ($greetMediaFiles as $greetMediaFile) {
                        ...
                        if ($greetMediaType == 'image') {
                            ...
                            $command = 'mkdir -p ' . $rootPath . $greetId . '/imageVideos && ' . $ffmpegBinPath . ' -loop 1 -i ' . $greetMedia . ' -f lavfi -i anullsrc -preset faster -c:v libx264 -c:a libfdk_aac -ac 2 -vf "' . $ratioSetting . 'pad=ceil(iw/2)*2:ceil(ih/2)*2" -t 5 -pix_fmt yuv420p -shortest ' . $imageVideoPath;
                            exec($command);
                            ...
                            if ($transition == 'zoompan') {
                                exec($ffmpegBinPath . " -loop 1 -i " . $greetMedia . " -f lavfi -i anullsrc -vf \"scale=-1:10*ih,zoompan=z='min(zoom+0.0015,1.5)':d=125:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=" . $imageVideoWidth . "x" . $imageVideoHeight . "\" -preset faster -c:v libx264 -c:a libfdk_aac -ac 2 -t 5 -pix_fmt yuv420p -shortest -y " . $mergedVideoPath);
                                ...
                                exec($ffmpegBinPath . ' -f lavfi -i color=black@0.0:d=1 -i ' . $imageVideoPath . ' -f lavfi -i color=black@0.0:d=1 -filter_complex "[0:v]scale=' . $imageVideoWidth . ':' . $imageVideoHeight . ',setsar=1,fps=fps=25  [color];[1:v]fps=fps=25 [video];[2:v]scale=' . $imageVideoWidth . ':' . $imageVideoHeight . ',setsar=1,fps=fps=25 [end];[color][video]xfade=transition=' . $transition . ':duration=1:offset=0,format=yuva420p [begin];[begin][end]xfade=transition=' . $transition . ':duration=1:offset=4,format=yuva420p" ' . $mergedVideoPath);
                            } else {
                                exec($ffmpegBinPath . ' -i ' . $imageVideoPath . ' ' . $mergedVideoPath);
                            }
                            if ($isTheme) {
                                exec($ffmpegBinPath . ' -i ' . $backgroundVideoPath . ' -i ' . $mergedVideoPath . ' -filter_complex "overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2:shortest=1,setsar=1,format=yuva420p" ' . $transparentVideoPath);
                            } else {
                                exec($ffmpegBinPath . ' -i ' . $mergedVideoPath . ' -vf "pad=1920:1080:(1920-iw)/2:(1080-ih)/2" ' . $transparentVideoPath);
                            }
                            ...
                            exec($ffprobeBinPath . ' -v error -select_streams v:0 -show_entries stream_tags=rotate -of default=nw=1:nk=1 ' . $greetMedia, $rotation);
                            $rotateFilter = '';
                            ...
                            if ($isTheme) {
                                if ($ratioSetting == '') {
                                    exec($ffmpegBinPath . ' -i ' . $greetMedia . $rotateFilter . ' -preset faster -c:v libx264 -pix_fmt yuv420p ' . $resizedVideoPath);
                                } else {
                                    exec($ffmpegBinPath . ' -i ' . $greetMedia . $rotateFilter . ' -vf "' . $ratioSetting . '" -preset faster -c:v libx264 -pix_fmt yuv420p ' . $resizedVideoPath);
                                }
                            } else {
                                exec($ffmpegBinPath . ' -i ' . $greetMedia . $rotateFilter . ' -vf "scale=1920:1080:force_original_aspect_ratio=decrease,setsar=1,pad=1920:1080:(ow-iw)/2:(oh-ih)/2" ' . $resizedVideoPath);
                            }
                            ...
                            if ($isTransition) {
                                ...
                                if ($isTheme) {
                                    if ($transition == 'zoompan') {
                                        exec($ffmpegBinPath . " -i " . $backgroundVideoPath . " -i " . $resizedVideoPath . " -filter_complex \"[1:v]scale=-1:10*ih,zoompan=z=pzoom+0.0015:x='iw/2-iw/zoom/2':y='ih/2-ih/zoom/2':d=1:s=" . $mediaWidth . "x" . $mediaHeight . ":fps=25[zoompanned];[0:v][zoompanned]overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2\" " . $transparentVideoPath);
                                    } else {
                                        exec($ffmpegBinPath . ' -f lavfi -i color=black@0.0:d=1 -i ' . $resizedVideoPath . ' -f lavfi -i color=black@0.0:d=1 -i ' . storage_path('app/public/theme_image/' . $greetTheme->file_name) . ' -filter_complex "[0:v]scale=' . $mediaWidth . ':' . $mediaHeight . ',fps=fps=25 [color];[1:v]fps=fps=25 [video];[2:v]scale=' . $mediaWidth . ':' . $mediaHeight . ',fps=fps=25 [end];[3:v]scale=2400:1600,setsar=1 [bg]; [color][video]xfade=transition=' . $transition . ':duration=1:offset=0,format=yuva420p [begin];[begin][end]xfade=transition=' . $transition . ':duration=1:offset=' . $duration - 1 . ',format=yuva420p[xfade]; [bg][xfade]overlay=(W-w)/2:(H-h)/2" ' . $transparentVideoPath);
                                    }
                                } else {
                                    if ($transition == 'zoompan') {
                                        exec($ffmpegBinPath . " -i " . $resizedVideoPath . " -vf \"scale=-1:10*ih,zoompan=z=pzoom+0.0015:x='iw/2-iw/zoom/2':y='ih/2-ih/zoom/2':d=1:s=" . $mediaWidth . "x" . $mediaHeight . ":fps=30 \" " . $transparentVideoPath);
                                    } else {
                                        exec($ffmpegBinPath . ' -f lavfi -i color=black@0.0:d=1 -i ' . $resizedVideoPath . ' -f lavfi -i color=black@0.0:d=1 -filter_complex "[0:v]scale=' . $mediaWidth . ':' . $mediaHeight . ',fps=fps=25  [color];[1:v]fps=fps=25 [video];[2:v]scale=' . $mediaWidth . ':' . $mediaHeight . ',fps=fps=25 [end];[color][video]xfade=transition=' . $transition . ':duration=1:offset=0,format=yuva420p [begin];[begin][end]xfade=transition=' . $transition . ':duration=1:offset=' . $duration - 1 . ',format=yuva420p" ' . $transparentVideoPath);
                                    }
                                }
                            } else {
                                if ($isTheme) {
                                    exec($ffmpegBinPath . ' -stream_loop -1 -i ' . $backgroundVideoPath . ' -i ' . $resizedVideoPath . ' -filter_complex "overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2:shortest=1,setsar=1,format=yuva420p" -r 25 ' . $transparentVideoPath);
                                } else {
                                    exec($ffmpegBinPath . ' -i ' . $resizedVideoPath . ' -vf "pad=1920:1080:(1920-iw)/2:(1080-ih)/2" -r 25 ' . $transparentVideoPath);
                                }
                            }
                            */
                            ...
                            if ($isTheme) {
                                var_dump('Converting video with theme to transparent video for ', $greetMediaName, $ffmpegBinPath . ' -stream_loop -1 -i ' . $backgroundVideoPath . ' -i ' . $resizedVideoPath . ' -filter_complex "overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2:shortest=1,setsar=1,format=yuva420p" -r 25 -ac 2 ' . $transparentVideoPath);
                                exec($ffmpegBinPath . ' -stream_loop -1 -i ' . $backgroundVideoPath . ' -i ' . $resizedVideoPath . ' -filter_complex "overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2:shortest=1,setsar=1,format=yuva420p" -r 25 -ac 2 ' . $transparentVideoPath);
                            } else {
                                var_dump('Converting video without theme to transparent video for ', $greetMediaName, $ffmpegBinPath . ' -i ' . $resizedVideoPath . ' -vf "pad=1920:1080:(1920-iw)/2:(1080-ih)/2" -r 25 -ac 2 ' . $transparentVideoPath);
                                exec($ffmpegBinPath . ' -i ' . $resizedVideoPath . ' -vf "pad=1920:1080:(1920-iw)/2:(1080-ih)/2" -r 25 -ac 2 ' . $transparentVideoPath);
                            }
                        }
                    }
                    ...
                    if (file_exists($rootPath . $greetId . '/list.txt')) {
                        exec($ffmpegBinPath . ' -f concat -safe 0 -i ' . $rootPath . $greetId . '/list.txt -preset faster -c:v libx264 -c:a libfdk_aac -ac 2 ' . $prePath);
                        $output = shell_exec($ffprobeBinPath . " -hide_banner -loglevel info -show_streams -select_streams a $prePath 2>&1");
                        if (strpos($output, 'Audio:') == false) {
                            var_dump('No Audio in prePath');
                            exec($ffmpegBinPath . ' -f lavfi -i anullsrc=channel_layout=stereo:sample_rate=44100 -i ' . $prePath . ' -c:v copy -c:a libfdk_aac -shortest -ac 2 ' . $preFinalPath);
                        } else {
                        ...
                        if ($isThemeMusic) {
                            ...
                            exec($ffmpegBinPath . ' -stream_loop -1 -i ' . $audioFile . ' -t ' . $currentLength . ' -c:a libfdk_aac -ac 2 ' . $longAudioPath);
                            if (empty($muteFilter)) {
                                exec($ffmpegBinPath . ' -i ' . $longAudioPath . ' ' . $finalAudioPath);
                            } else {
                                $muteFilter = substr($muteFilter, 0, -2);
                                exec($ffmpegBinPath . ' -i ' . $longAudioPath . ' -af "' . $muteFilter . '" ' . $finalAudioPath);
                            }
                            exec($ffmpegBinPath . ' -i ' . $preFinalPath . ' -stream_loop -1 -i ' . $finalAudioPath . ' -filter_complex "[0:a][1:a]amix=duration=first:dropout_transition=3,volume=1[aout]" -map 0:v -map "[aout]" -c:v copy -c:a libfdk_aac -ac 2 -shortest ' . $finalVideoPath, $output, $retval);
                        } else {
                            exec($ffmpegBinPath . ' -f concat -safe 0 -i ' . $rootPath . $greetId . '/list.txt -preset faster -c:v libx264 -c:a libfdk_aac -ac 2 ' . $finalVideoPath, $output, $retval);
                        }
                    }
                    ...
                    $timecmd = exec($ffmpegBinPath . ' -i ' . $finalVideoPath . ' 2>&1 | grep Duration | cut -d " " -f 4 | sed s/,//');
                    ...
                    if ($greet->occasion_name == 'Sweet Greet' && $duration > 3) {
                        $trim_cmd = $ffmpegBinPath . ' -i ' . $finalVideoPath . ' -ss 00:00:00 -t 00:03:00 -c:v copy -c:a copy ' . $trimmedFinalVideoPath;
                    ...
                    if ($duration > 45) {
                        $trim_cmd = $ffmpegBinPath . ' -i ' . $finalVideoPath . ' -ss 00:00:00 -t 00:45:00 -c:v copy -c:a copy ' . $trimmedFinalVideoPath;
                    ...
                    exec($ffmpegBinPath . ' -i ' . $finalVideoPath . ' -i ' . $img . ' -filter_complex "overlay=W/2-w/2:H/2-h/2" -movflags +faststart ' . $previewVideoPath, $output, $retval);
                    ...
                    if ($retval == 0) {
                        ...
                        $video_link_preview = ('storage/greetMedia/final/' . $greetId . '/' . $rdname . 'preview.mp4');
                        $data['link'] = $video_link_preview;



                        if ($greet->occasion_name == 'Sweet Greet' && $duration <= 3) {
                            $data['link'] = $dbfinalvideopath . $rdname . 'trimmed_final.mp4';
                            $video_link_f = ('storage/greetMedia/final/' . $greetId . '/' . $rdname . 'trimmed_final.mp4');
                            $data['link'] = $video_link_f;
                        }
                    ...
                    $commandMessage = 'Video Creation Failed - Server Error';
                    $greetMediaRequestArr = [
                        'comments' => $commandMessage,
                        'status' => 3
                    ];
                    $requestedGreet->update($greetMediaRequestArr);
                    throw $e;
                }
            } else {
